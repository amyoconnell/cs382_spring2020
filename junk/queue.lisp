...
;; ====================================
;;  This file contains an implementation of priority queues.
;;  based on queue implementation by Prof. Nancy E. Reed
;;  http://www2.hawaii.edu/~nreed/aima/utilities/queue.lisp
;; =========================================================

;;;; The Queue datatype

;;; We can remove elements from the front of a queue.  We can add elements in
...
(defun heap-sort (numbers &key (key #'identity))
  ...
  (let ((heap (make-heap))
        (result nil))
    (dolist (n numbers) (heap-insert heap n key))
    (while (> (length heap) 0) (push (heap-extract-min heap key) result))
    (nreverse result)))

;; (heap-sort (list '(2 F) '(1 G) '(5 P) '(6 C) '(0 C)) :key #'first)
