...
(defstruct edge
  ...
  succ-index
  pred-index
  (round *init-round*) ;; used while propagating pl-funcs
  )
...
(defun make-edges-etc (&key (init-max-n *n-incr*)
  ...
  (let ((init-size (list init-max-n init-max-n)))
    (make-instance 'edges-etc
      ...
      :dist (if include-dist
		(make-dist-mat init-max-n)
	      nil))))
...
(defun make-uc-edges-etc (&key (init-max-n *n-incr*)
			       (init-max-k *k-incr*))
...
(defclass nu-stnu ()
  ...
  (
   ...
   (num-tps :accessor num-tps :initform 0)

   ;;  TP-HASH
   ;;    if KEY is a number (index), then VALUE is the corresponding SYMBOL
   ;;    if KEY is a symbol (name), then VALUE is the corresponding number

   (tp-hash :accessor tp-hash :initform (make-hash-table))
   ...
   (num-cls :accessor num-cls :initform 0)

   ;;  CL-VEC, a vector containing contingent link structs

   (cl-vec :accessor cl-vec
	   :initform (make-a-vector *k-incr*)
	   :initarg :cl-vec)
   ...
   (cl-index-vec :accessor cl-index-vec
		 :initform (make-a-vector *n-incr*)
   ...
   (ord-edges-etc :accessor ord-edges-etc
   ...
   (dmax-edges-etc :accessor dmax-edges-etc
		   :initform (make-edges-etc)
   ...
   (uc-edges-etc :accessor uc-edges-etc
		 :initform (make-uc-edges-etc :init-max-n *n-incr*
					      :init-max-k *k-incr*)
...
(defun make-nu-stnu (&key (init-max-n *n-incr*)
			  (init-max-k *k-incr*)
			  (include-dist t)
			  (include-dmax t))
  (let () ;; ((init-size (list init-max-n init-max-n)))
    (make-instance 'nu-stnu
      ...
      :ord-edges-etc (make-edges-etc :init-max-n init-max-n
				     :include-dist include-dist)
      :dmax-edges-etc (if include-dmax
			  (make-edges-etc :init-max-n init-max-n
					  :include-dist include-dist)
			nil)
      :uc-edges-etc (make-uc-edges-etc :init-max-n init-max-n
				       :init-max-k init-max-k)
